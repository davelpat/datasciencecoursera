...
```{r load libs}
packages <- c("data.table", "dplyr", "lubridate", "stringr", "ggplot2", "grid", "gridExtra")
...
tt_plot(tte, "Top Ten Most Frequent Events")
...
ttf <- tbl_df(aggregate(FATALITIES ~ EVTYPE, storm_data, sum)) %>% 
    ...
    head(10)
ttfp <- tt_plot(ttf, "Top Ten Most Fatal Events")
ttfp
...
tti <- tbl_df(aggregate(INJURIES ~ EVTYPE, storm_data, sum)) %>% 
    ...
    head(10)
ttip <- tt_plot(tti, "Top Ten Most Injurious Events")
ttip

# Print both graphs as a single plot
## this approach generates this error:
## Error in gList(list(grobs = list(list(x = 0.5, y = 0.5, width = 1, height = 1,  : 
##  only 'grobs' allowed in "gList"
#grid.arrange(ttfp, ttip, nrow = 2, main = "Health Impacts of Storms")
```
...
ttp <- tbl_df(aggregate(PROPDMGNORM ~ EVTYPE, storm_data, sum)) %>% 
    ...
    head(10)
ttpp <- tt_plot(ttp, "Top Ten Events Causing the Most Property Damage\nBillions of Dollars")
ttpp
...
ttc <- tbl_df(aggregate(CROPDMGNORM ~ EVTYPE, storm_data, sum)) %>% 
    ...
    head(10)
ttcp <- tt_plot(ttc, "Top Ten Events Causing the Most Crop Damage\nBillions of Dollars")
ttcp
```
